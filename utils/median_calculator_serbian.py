import pandas as pd
import numpy as np
from utils import csv_handler, constant
...
def calculate_median_per_column_value(column_data, column_median_list, column_name):
    column_data_array = column_data[constant.CENA].to_numpy()
...
def get_grouped_data_by_column(column):
    switcher = {
        # MARKA: data[MARKA],
        constant.MODEL: data[constant.MODEL],
        constant.GODISTE: data[constant.GODISTE],
        constant.KILOMETRAZA: data[constant.KILOMETRAZA],
        constant.KAROSERIJA: data[constant.KAROSERIJA],
        constant.GORIVO: data[constant.GORIVO],
        constant.KUBIKAZA: data[constant.KUBIKAZA],
        constant.SNAGA_MOTORA: data[constant.SNAGA_MOTORA],
        constant.EMISIONA_KLASA_MOTORA: data[constant.EMISIONA_KLASA_MOTORA],
        constant.POGON: data[constant.POGON],
        constant.MENJAC: data[constant.MENJAC],
        constant.CENA: data[constant.CENA]
    }
...
def populate_median_lists():
    ...
    calculate_median(constant.MODEL, model_median_list)
    calculate_median(constant.GODISTE, year_median_list)
    calculate_median(constant.KILOMETRAZA, mileage_median_list)
    # calculate_median(KAROSERIJA, car_body_median_list)
    # calculate_median(GORIVO, fuel_type_median_list)
    calculate_median(constant.KUBIKAZA, engine_size_median_list)
    calculate_median(constant.SNAGA_MOTORA, engine_power_median_list)
...
def fill_all_median_list():
    ...
    all_median_list[constant.MODEL] = model_median_list
    all_median_list[constant.GODISTE] = year_median_list
    all_median_list[constant.KILOMETRAZA] = mileage_median_list
    # all_median_list[KAROSERIJA] = car_body_median_list
    # all_median_list[GORIVO] = fuel_type_median_list
    all_median_list[constant.KUBIKAZA] = engine_size_median_list
    all_median_list[constant.SNAGA_MOTORA] = engine_power_median_list
